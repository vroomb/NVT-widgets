    ...
    QApplication(argc, argv)
{
    ...
    if (johnston_bold_id != -1) for (auto i : QFontDatabase::applicationFontFamilies(johnston_bold_id))
        log("loaded " + i.toStdString());
    else log("could not load johnston bold itc");

    log("loading johnston medium itc");
    if (johnston_medium_id != -1) for (auto i : QFontDatabase::applicationFontFamilies(johnston_medium_id))
        log("loaded " + i.toStdString());
    ...
    auto thing  = new QTextEdit();
    w.setCentralWidget(thing);
}
